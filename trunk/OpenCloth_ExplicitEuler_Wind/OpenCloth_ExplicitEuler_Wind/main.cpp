...
void OnRender() {		
	...
	sprintf_s(info, "FPS: %3.2f, Frame time (GLUT): %3.4f msecs, Frame time (QP): %3.3f, Wind: %1.0f", fps, frameTime, frameTimeQP, bWind);
	...
	glBegin(GL_LINES);
		...
		glVertex3f(0,0,0);
		glm::vec3 dir = glm::normalize(wind*bWind);
...
void StepPhysics(float dt ) {
	...
	wind+=glm::vec3(rx,ry,rz)*dt*0.001f;
...
void main(int argc, char** argv) {
	...
	glutCreateWindow("GLUT Cloth Demo [Explicit Euler Integration + Wind]");
